...
function showFriend() {
	...
	//将数据放入二维数组	
	for(var i = 0; i < concernList.length; i++) {
		if(concernList[i][1] != "" && concernList[i][1] != null) {
	...
	while(i < concernList.length) {
		...
		if(j >= "0" && j <= "9") {
			...
			$("#friendList").append("<li class='friend mui-table-view-cell mui-indexed-list-item' id='" + pinyin[i][2] + "'>" + pinyin[i][1] + "</li>");
		} else if(j == "a" || j == "A") {
			if(tag != "a") {
		...
		success: function(data, textStatus) {
			...
			if(errCode == 0) {
				...
				for(var i = 0; i < data.data.length; i++) {
					concernList[i] = new Array();
					concernList[i][0] = data.data[i].userId;
					concernList[i][1] = data.data[i].remarkName;
					concernList[i][2] = data.data[i].nickname;
